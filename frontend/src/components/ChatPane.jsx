...
function isJacobQuestion(question) {
  const q = question.toLowerCase()
  return q.includes('who is jacob') || q.includes('who is the expert') || q.includes('contact jacob')
}
...
export default function ChatPane({ capturedProcedures = [], initialMessages, onMessagesChange, onSelectExpert, onRequestCreated, onGoToInbox }) {
  ...
  function answerFor(question) {
    ...
    if (isJacobQuestion(question)) {
      return {
        role: 'assistant',
        kind: 'agent',
        content: {
          message:
            '- Jacob Keller is the SME behind the captured Master Data workflow [1]\n- He specializes in ESG/SFDR verification and sub-classification checks [1]\n- Use Contact Expert for manual escalation when the procedure is incomplete',
          requires_bpo_action: false,
          bpo_draft_form: null,
          document_citations: [],
          expert_citations: [JACOB_PROFILE],
          sources: [],
        },
      }
    }
  ...
  async function submit(text) {
    ...
    // Primary path: the knowledge & context engine (specs 09/10/11).
    try {
      ...
      // fall through to the legacy agent/rag path below
    }

    // Fallback path: Phase-2 agent → RAG → local demo.
    try {
  ...
  return String(text).split(/(\[\d+\])/g).map((part, i) => {
    ...
    return (
      <Cite
        key={i}
        n={n}
        source={sources.find(s => s.index === n)}
        onClick={onCiteClick}
      />
    )
  })
        ...
        <ul className="space-y-1.5">
          {bullets.map((b, i) => (
            <li key={i} className="flex gap-2 text-xs text-ink leading-relaxed">
              ...
              <span>
                <InlineCitations text={b} sources={sources} onCiteClick={onCiteClick} />
              </span>
            </li>
          ))}
        </ul>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {experts.map((expert) => (
          <article
            ...
            className="w-full max-w-[230px] rounded-xl border border-six/25 bg-six-light/60 p-3"
          >
